...
function tickRoom(room) {
  ...
  if (event?.type === "powerup") handlePowerUp(room, event);
...
async function handlePowerUp(room, event) {
  const { effectType, beneficiary, target } = event;
  if (!beneficiary) return;
  const player = room.playerBySlot(beneficiary);
  if (!player) return;
  ...
  try {
    ...
    io.to(room.code).emit("powerup_triggered", { effectType, beneficiary, target });
  } catch (err) {
    console.warn(`Power-up payment failed for ${player.name} in room ${room.code}:`, err.message);
    if (room.outOfFundsMode === "draw") {
      io.to(room.code).emit("payment_failed", { slot: beneficiary, message: err.message });
      handleDraw(room);
    } else if (room.outOfFundsMode === "pause") {
      room.pauseFor(beneficiary);
      io.to(room.code).emit("payment_paused", { slot: beneficiary, message: err.message, hitFeeSats: fee });
      io.to(room.code).emit("state", room.publicState());
    } else {
      io.to(room.code).emit("payment_failed", { slot: beneficiary, message: err.message });
      handleMiss(room, beneficiary);
    }
  }
}
